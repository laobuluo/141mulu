...
if ($action == 'list') {
	...
	$where = '';
	$sql = "SELECT a.web_id, a.user_id, a.cate_id, a.web_name, a.web_url, a.web_istop, a.web_isbest, a.web_status, a.web_ctime, b.web_ip, b.web_grank, b.web_brank, b.web_srank, b.web_arank, b.web_instat, b.web_outstat, b.web_views, b.web_errors, c.cate_name, u.nick_name FROM $table a LEFT JOIN ".$DB->table('webdata')." b ON a.web_id=b.web_id LEFT JOIN ".$DB->table('categories')." c ON a.cate_id=c.cate_id LEFT JOIN ".$DB->table('users')." u ON a.user_id=u.user_id WHERE";
	...
	while ($web = $DB->fetch_array($query)) {
		...
		$web['web_attr'] = $web_istop.' - '.$web_isbest.' - '.$web_status;
